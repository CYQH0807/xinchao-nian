...
- **3.3.6**：MCP 关系类自报默认需要 `exchange`；自助类念头提示与事件类型逐维对应；同一念头只回推一次、驱力回推封顶 0.85。白昼/念头/远期梦材料改走无 query 的近期浮现路径，清理核心准则和技术域；参数按本地 Ombre 3.6.3 工具签名适配，不传 `mode` / `with_ids`。
...
| `xinchao_event` | 回传一次明确互动及有界窗口状态；`event_id` 用于幂等。自己窗口里直接填的类型只认 sharing / reflection / task_progress / discovery；关系类互动提供 `exchange` 让服务端判 |
