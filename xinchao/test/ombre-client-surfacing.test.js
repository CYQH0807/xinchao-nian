...
test('automatic recall uses the local Ombre 3.6.3 no-query tool contract', async () => {
  const client = new OmbreClient({ breathMaxResults: 3, breathMaxTokens: 800 });
  const calls = [];
  client.call = async (name, args) => {
    calls.push({ name, args });
    return { result: { content: [{ type: 'text', text: '[bucket_id:abcdef123456] [domain:生活]\n一条具体记忆' }] } };
  };
  const now = new Date('2026-09-18T12:00:00.000Z');
  const emotion = { valence: 0.8, arousal: 0.4 };

  await client.daytimeMaterialWithRefs([], emotion, now);
  await client.thoughtMaterialWithRefs([], emotion, now);
  await client.farMaterial(now);

  assert.deepEqual(calls, [
    { name: 'breath_advanced', args: { valence: 0.8, arousal: 0.4, date_from: '2026-09-04', max_results: 3, max_tokens: 12000 } },
    { name: 'breath_advanced', args: { valence: 0.8, arousal: 0.4, date_from: '2026-09-04', max_results: 3, max_tokens: 9000 } },
    { name: 'breath_advanced', args: { max_results: 1, max_tokens: 10000, date_to: '2026-08-19' } },
  ]);
