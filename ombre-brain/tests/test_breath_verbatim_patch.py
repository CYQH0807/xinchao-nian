...
def test_verbatim_render_exposes_affinity_metadata_without_rewriting_body():
    bucket = {
        "content": "原始正文 [[显示名]] 仍然保留。",
        "metadata": {"domain": ["恋爱"], "tags": "承诺,共同经历"},
    }

    rendered, _ = render_stored_bucket(bucket, "[bucket_id:affinity-test]")

    assert "[domain:恋爱] [tags:承诺,共同经历]" in rendered
    assert "原始正文 显示名 仍然保留。" in rendered
